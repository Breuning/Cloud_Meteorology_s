...
const char *ATCmds[] =
{
	...
	"AT+QMTOPEN=0,\"139.196.208.106\",61613\r\n",
//	"AT+QMTCONN=0,\"clientExample\",\"admin\",\"ibs_admin\"\r\n",
	...
	"AT+QMTPUB=0,0,0,1,\"mqttTopicMeteoro\"\r\n",
...
void NBiot_Task()
{
	char *Ack_QMTOPENTEST = "+QMTOPEN: 0,\"139.196.208.106\",61613";
	char *Ack_QMTCONNTEST = "+QMTCONN: 0,3";
	char *a = NULL, *b = NULL;

	if(NBiotTaskTimerFlag == TRUE)              //通过TIM5设置为NBiot每3min上传一次数据包
	{
...
//	HAL_GetUID(UID);   //读取芯片的96位唯一标识ID
	...
	sprintf((char*)MCUID, "%0*lx", 8, HAL_GetUIDw0());
	sprintf((char*)MCUID + 8, "%0*lx", 8, HAL_GetUIDw1());
	sprintf((char*)MCUID + 16, "%0*lx", 8, HAL_GetUIDw2());
}
